...
!function () {
    if (isWindows = -1 < navigator.platform.indexOf("Win"), isWindows) {
        new PerfectScrollbar(".sidebar"), new PerfectScrollbar(".sidebar-wrapper"), new PerfectScrollbar(".main-panel");
        $("html").addClass("perfect-scrollbar-on")
    } else $("html").addClass("perfect-scrollbar-off")
}(), backgroundOrange = navbar_initialized = fixedTop = !(transparentDemo = transparent = !0), toggle_initialized = sidebar_mini_active = !1, seq = 0, delays = 80, durations = 500, seq2 = 0, delays2 = 80, durations2 = 500, $(document).ready(function () {
    ...
    misc: {navbar_menu_visible: 0}, initMinimizeSidebar: function () {
        ...
        try {
            $.notify({icon: "now-ui-icons ui-1_bell-53", message: a}, {
                type: "info",
                timer: 4e3,
